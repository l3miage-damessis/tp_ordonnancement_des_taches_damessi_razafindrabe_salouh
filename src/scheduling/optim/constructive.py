...
class NonDeterminist(Heuristic):
    ...
    def run(self, instance: Instance, params: Dict = dict()) -> Solution:
        ...
        while scheduled_operations < total_operations:
            current_available_operations = list(solution.available_operations)
            if not current_available_operations:
                print("Warning: Deadlock or unfeasible instance detected.")
                break

            random.shuffle(current_available_operations) 

            options: List[Tuple[Operation, Machine, float, float]] = []

            for op in current_available_operations:
...
if __name__ == "__main__":
    ...
    inst = Instance.from_file(TEST_FOLDER_DATA + os.path.sep + "jsp2")
